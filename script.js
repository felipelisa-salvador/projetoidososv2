document.addEventListener('DOMContentLoaded', () => {
    ...
    const firebaseConfig = {
        ...
        databaseURL: "https://fumpres-coracao-default-rtdb.firebaseio.com/",
    ...
    // --- 1. FUNÇÃO PARA BUSCAR E MOSTRAR A LISTA (USANDO FIREBASE) ---
    function loadIdosos() {
        ...
        IDOSOS_REF.once('value', snapshot => {
            ...
            // Converte o objeto de idosos em um array para fácil iteração
            ...
            idososArray.forEach(idoso => {
                ...
                card.className = 'card';
                card.dataset.id = idoso.id; // O ID é a chave do Firebase
    ...
    // --- 2. FUNÇÃO DE ENVIO DO FORMULÁRIO (USANDO FIREBASE TRANSACTIONS) ---
    form.addEventListener('submit', async (e) => {
        ...
        // Usa Promise.all para processar todos os idosos selecionados em paralelo
        const updatePromises = selectedIdosos.map(idosoId => {
            ...
            return idosoRef.child('count').transaction(currentCount => {
                const count = currentCount || 0; // Trata 'null' ou 'undefined' como 0
                
                if (count < LIMITE_PRESENTES) {
                    // O retorno é o novo valor a ser escrito no banco
                    return count + 1; 
                } else {
                    // Retornar 'undefined' aborta a transação e não escreve nada
                    return; 
                }
            }).then(result => {
                if (result.committed) {
        ...
        try {
            // Espera todas as transações serem concluídas
            await Promise.all(updatePromises);
            
            if (successfulChoices > 0) {
                // Se houver sucesso, salva o log do doador
